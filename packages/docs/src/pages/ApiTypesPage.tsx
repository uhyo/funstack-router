  ...
  return (
    <div className="page docs-page api-page">
      ...
      <article className="api-item">
        ...
        <p>
          Props type for route components without a loader. Includes navigation state management
          props.
        </p>
  ...
  isPending: boolean; // Whether a navigation transition is pending
        ...
        <ul>
          ...
          <li>
            ...
            <code>navigation.updateCurrentEntry()</code>. This is <strong>not</strong> a navigation,
            so it bypasses React transitions and will never set <code>isPending</code> to{" "}
            <code>true</code>.
          </li>
          <li>
            <code>resetState</code> - Async method that clears navigation state via replace
            navigation. Like <code>setState</code>, it goes through a React transition and may set{" "}
            <code>isPending</code> to <code>true</code>.
          </li>
          <li>
            <code>resetStateSync</code> - Clears navigation state synchronously. Like{" "}
            <code>setStateSync</code>, this bypasses React transitions and will never set{" "}
            <code>isPending</code> to <code>true</code>.
          </li>
        </ul>
      </article>
      ...
      <article className="api-item">
        ...
        <p>
          Props type for route components with a loader. Extends <code>RouteComponentProps</code>{" "}
          with a <code>data</code> prop.
        </p>
...
type Props = {
  ...
  isPending: boolean; // Whether a navigation transition is pending
      ...
      <article className="api-item">
        ...
        <p>Utility type that extracts parameter types from a path pattern string.</p>
...
// { userId: string }`}</CodeBlock>
      ...
      <article className="api-item">
        <h3>
          <code>TypefulOpaqueRouteDefinition&lt;Id, Params, State, Data&gt;</code>
        </h3>
        <p>
          A route definition that carries type information. Created when using <code>route()</code>{" "}
          or <code>routeState()</code> with an <code>id</code> property. This enables type-safe
          access to route params, state, and data via hooks.
        </p>
...
// Type: OpaqueRouteDefinition`}</CodeBlock>
      ...
      <article className="api-item">
        ...
        <p>
          Helper types to extract type information from <code>TypefulOpaqueRouteDefinition</code>.
...
// { name: string; age: number }`}</CodeBlock>
      ...
      <article className="api-item">
        ...
        <p>
          Utility type that extracts the component props type from a route definition. Returns{" "}
          <code>RouteComponentProps</code> for routes without a loader, or{" "}
          <code>RouteComponentPropsWithData</code> for routes with a loader. This is useful for
          typing route components separately from the route definition.
        </p>
        <CodeBlock language="tsx">{`import { route, routeState } from "@funstack/router";
...
// RouteComponentProps<Record<string, never>, MyState>`}</CodeBlock>
        <p>
          <strong>Note:</strong> This utility requires a route with an <code>id</code> property.
          Using it with a route without <code>id</code> will result in a type error.
        </p>
      </article>
      ...
      <article className="api-item">
        ...
        <ul>
          <li>
            <strong>Component reference</strong> (e.g., <code>component: UserPage</code>): Router
            automatically injects props (<code>params</code>, <code>state</code>,{" "}
            <code>setState</code>, <code>setStateSync</code>, <code>resetState</code>,{" "}
            <code>resetStateSync</code>, <code>info</code>, <code>isPending</code>, and{" "}
            <code>data</code> when a loader is defined).
          </li>
          <li>
            <strong>JSX element</strong> (e.g., <code>component: &lt;UserPage /&gt;</code>):
            Rendered as-is without router props injection. Useful for static components or when you
...
routeState<{ tab: string }>()({
  ...
  loader: () => fetchUser(),
      ...
      <article className="api-item">
        ...
        <p>
          Arguments passed to route action functions. The <code>request</code> carries the POST
          method and <code>FormData</code> body from the form submission.
        </p>
  ...
  signal: AbortSignal;
      ...
      <article className="api-item">
        ...
        <p>
          Arguments passed to route loader functions. The optional <code>actionResult</code>{" "}
          parameter contains the return value of the route's action when the loader runs after a
          form submission.
        </p>
  ...
  actionResult: ActionResult | undefined;
        ...
        <p>
          On normal navigations, <code>actionResult</code> is <code>undefined</code>. After a form
          submission, it contains the action's return value (awaited if the action is async).
        </p>
      </article>
  ...
  entryKey: string | null;  // NavigationHistoryEntry.key
        ...
        <p>
          <code>entryId</code> and <code>entryKey</code> expose the corresponding properties from
          the Navigation API's <code>NavigationHistoryEntry</code>. <code>entryId</code> is a unique
          identifier for the entry — a new id is assigned when the entry is replaced.{" "}
          <code>entryKey</code> represents the slot in the entry list and is stable across
          replacements. Both are <code>null</code> when the Navigation API is unavailable (e.g., in
          static fallback mode).
        </p>
        <p>
          <strong>Warning:</strong> Do not render these values directly in DOM, as they are not
          available during SSR and will cause a hydration mismatch. They are best suited for use as
          a React <code>key</code> or in effects/callbacks.
        </p>
      </article>
  ...
  info?: unknown;  // Ephemeral, not persisted in history
        ...
        <p>
          <strong>Note:</strong> <code>state</code> is persisted in history and available across
          back/forward navigation. <code>info</code> is ephemeral and only available during the
