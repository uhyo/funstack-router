...
function MyComponent() {
  ...
  console.log(location.entryKey);  // NavigationHistoryEntry.key or null
        ...
        <p>
          <code>entryId</code> and <code>entryKey</code> are <code>null</code> when the Navigation
          API is unavailable. Do not render them directly in DOM — they are not available during SSR
          and will cause a hydration mismatch. Use them as a React <code>key</code> or in
          effects/callbacks instead.
        </p>
      </article>
      ...
      <article className="api-item">
        ...
        <p>Returns a tuple of the current search params and a setter function.</p>
...
setSearchParams({ q: newQuery }, { replace: false });`}</CodeBlock>
      ...
      <article className="api-item">
        ...
        <p>
          Prevents navigation away from the current route. Useful for scenarios like unsaved form
          data, ongoing file uploads, or any state that would be lost on navigation.
        </p>
  ...
  return (
    <form>
      ...
      <button type="button" onClick={handleSave}>
        ...
        <ul>
          <li>
            <code>shouldBlock</code>: A function that returns <code>true</code> to block navigation,
            or <code>false</code> to allow it. You can call <code>confirm()</code> inside this
            function to show a confirmation dialog. Wrap with <code>useCallback</code> when the
        ...
        <ul>
          <li>
            ...
            <code>true</code>, navigation is blocked.
          </li>
          <li>
      ...
      <article className="api-item">
        ...
        <p>
          Returns whether a navigation transition is currently pending. When navigating to a route
          that suspends (e.g., using <code>React.lazy</code>), <code>isPending</code> becomes{" "}
          <code>true</code> while React keeps the previous UI visible, and returns to{" "}
          <code>false</code> once the new route is ready.
        </p>
  ...
  return (
    <div style={{ opacity: isPending ? 0.7 : 1 }}>
      ...
      {/* page content */}
        ...
        <ul>
          <li>
            <code>boolean</code> — <code>true</code> when a navigation transition is in progress
            (the destination route is suspending), <code>false</code> otherwise.
          </li>
        </ul>
        <h4>Notes</h4>
        <ul>
          <li>
            This hook is powered by React's <code>useTransition</code>. The router wraps navigations
            in <code>startTransition</code>, so React defers rendering suspended routes and keeps
            the current UI visible.
          </li>
          <li>
            Sync state updates via <code>setStateSync</code> and <code>resetStateSync</code> bypass
            transitions entirely, so <code>isPending</code> will <strong>not</strong> become{" "}
            <code>true</code> for those updates.
          </li>
          <li>
            The same <code>isPending</code> value is also available as a prop on route components.
          </li>
        </ul>
      </article>
      ...
      <p>
        ...
        example, a child route component can use <code>useRouteParams(parentRoute)</code> to access
        the parent route's parameters.
      </p>
      ...
      <article className="api-item">
        ...
        <p>
          Returns typed route parameters for the given route definition. The parameter types are
          automatically inferred from the route's path pattern.
        </p>
...
function TeamPage() {
  ...
  return <div>Org: {orgId}</div>;
        ...
        <ul>
          ...
          <li>
            Throws if the specified route's <code>id</code> is not found in the current route
            hierarchy (neither the current route nor any ancestor).
          </li>
        </ul>
      </article>
      ...
      <article className="api-item">
        ...
        <p>
          Returns typed navigation state for the given route definition. Use this with routes
          defined via <code>routeState&lt;T&gt;()</code> to get properly typed state.
        </p>
...
function ScrollPage() {
  ...
  return <div>Scroll position: {state?.scrollPos ?? 0}</div>;
        ...
        <h4>Return Value</h4>
        <p>
          Returns <code>State | undefined</code>. State is <code>undefined</code> on initial visit
          and when navigating to a new entry without state.
        </p>
        <h4>Errors</h4>
        <ul>
          <li>Throws if called outside a route component (no RouteContext).</li>
      ...
      <article className="api-item">
        ...
        <p>
          Returns typed loader data for the given route definition. The data type is automatically
          inferred from the route's <code>loader</code> function.
        </p>
